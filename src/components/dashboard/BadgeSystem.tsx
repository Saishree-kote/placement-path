...
const BadgeSystem = () => {
    ...
    const fetchStatsAndBadges = async () => {
        ...
        const { data: profile } = await supabase
            .from("profiles")
            .select("readiness_score, badges, full_name, skills, college")
        ...
        const newlyEarned = BADGES
            .filter((b) => b.condition(currentStats) && !alreadyEarned.includes(b.id))
            .map((b) => b.id);
    ...
    const handleShare = (badge: typeof BADGES[0]) => {
        ...
        if (navigator.share) {
            navigator.share({ title: "PlacePrep Badge", text });
        } else {
            navigator.clipboard.writeText(text);
            toast({ title: "Copied to clipboard!", description: "Share your achievement!" });
        }
    };
            ...
            <div className="flex items-center justify-between mb-4">
                ...
                <div className="flex gap-1">
                    {Array.from({ length: BADGES.length }).map((_, i) => (
                        <div
                            key={i}
                            className={"w-1.5 h-1.5 rounded-full transition-colors " + (i < earnedCount ? "bg-yellow-500" : "bg-muted")}
                        />
                    ))}
                </div>
            </div>
                    ...
                    {displayBadges.map((badge) => {
                        ...
                        return (
                            <motion.div
                                ...
                                className={"relative p-3 rounded-xl border transition-all group " + (isEarned ? badge.bgColor + " border-transparent" : "bg-muted/20 border-border/30 opacity-50")}
                            >
                                <div className="flex items-start gap-2">
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <p className={"text-xs font-semibold leading-tight " + (isEarned ? "text-foreground" : "text-muted-foreground")}>
                                            {badge.name}
                                        </p>
